...
/**
 * Return the accumulated duration of the specified timer, in seconds.
 *
 * timer: ID of the timer to query.
 */
real_t TimeKeeper::GetSeconds(const len_t timer) const {
    return timers[timer]->timer->GetMicroseconds() / 1e6;
}
