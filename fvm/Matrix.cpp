...
Matrix::Matrix(
    ...
    const PetscInt *nnzl) { this->Construct(m, n, nnz, nnzl); }
    ...
    const PetscInt *nnzl)
{
    ...
    // COLLECTIVE: every rank must reach this.
    if ((ierr = MatCreate(PETSC_COMM_WORLD, &this->petsc_mat)))
        throw MatrixException("Failed to create PETSc matrix. Error code: %d", ierr);

    // m, n are GLOBAL sizes; let PETSc choose the local split.
    MatSetSizes(this->petsc_mat, PETSC_DECIDE, PETSC_DECIDE, m, n);

    // MATAIJ resolves to SeqAIJ on 1 rank, MPIAIJ on more.
    MatSetType(this->petsc_mat, MATAIJ);
    MatSetUp(this->petsc_mat);
    ...
    // Off while the preallocation is unrefined -- otherwise every
    // unpreallocated insertion is a hard error.
    MatSetOption(this->petsc_mat, MAT_NEW_NONZERO_ALLOCATION_ERR, PETSC_FALSE);

    this->allocated = true;

    // Insertion may be one-sided. Each rank sets only the diagonals it owns;
    // stashing remote rows here would be legal but needlessly slow.
    PetscInt rstart, rend;
    MatGetOwnershipRange(this->petsc_mat, &rstart, &rend);
...
real_t *Matrix::Multiply(const len_t n, const real_t *f)
{
    ...
    VecAssemblyBegin(f_v);
    VecAssemblyEnd(f_v);
    VecAssemblyBegin(Af_v);
    VecAssemblyEnd(Af_v);
