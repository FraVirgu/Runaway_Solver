...
void SolverNonLinear::Allocate() {
    ...
    // The Newton state (petsc_F, petsc_dx) is replicated in full on every
    // process, alongside the raw x0/x1/dx arrays below which are indexed
    // over the whole system. They must therefore live on the
    // self-communicator (VECSEQ cannot be created on more than one process).
    VecCreateSeq(PETSC_COMM_SELF, N, &this->petsc_F);
    VecCreateSeq(PETSC_COMM_SELF, N, &this->petsc_dx);
