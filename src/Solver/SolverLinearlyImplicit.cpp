...
void SolverLinearlyImplicit::Solve(const real_t t, const real_t dt)
{
    ...
    do
    {
        ...
        MPI_Comm_size(PETSC_COMM_WORLD, &dbg_size);
        {
            ...
            printf("[%d] STEP %llu iter %llu | matrix_size=%llu | vec rows [%d,%d) | mat rows [%d,%d) | mat type=%s\n",
                   dbg_rank, (unsigned long long)this->nTimeStep,
