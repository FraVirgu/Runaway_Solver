...
KineticEquationTermIntegratedOverMomentum::KineticEquationTermIntegratedOverMomentum(
    ...
    FVM::UnknownQuantityHandler *u, bool rebuild, PetscScalar scaleFactor) : FVM::EquationTerm(fluidGrid), kineticGrid(kineticGrid),
...
void KineticEquationTermIntegratedOverMomentum::SetMatrixElements(FVM::Matrix *mat, real_t *rhs)
{
    ...
    MatScale(this->CsetElements, scaleFactor);

    PetscLogEvent ev;
    PetscLogEventRegister("KinIntegMomentum", MAT_CLASSID, &ev);
    PetscLogEventBegin(ev, 0, 0, 0, 0);

    const PetscInt rowOffset = mat->GetRowOffset();
    const PetscInt colOffset = mat->GetColOffset();
    for (PetscInt ir = 0; ir < (PetscInt)nr; ir++)
    {
        const PetscInt *cols;
        ...
        MatGetRow(this->CsetElements, ir, &ncols, &cols, &vals);

        PetscInt row = ir + rowOffset;
        offsetCols.resize(ncols);
        for (PetscInt k = 0; k < ncols; k++)
            offsetCols[k] = cols[k] + colOffset;

        MatSetValues(mat->mat(), 1, &row, ncols, offsetCols.data(), vals, ADD_VALUES);
        MatRestoreRow(this->CsetElements, ir, &ncols, &cols, &vals);
    }
    PetscLogEventEnd(ev, 0, 0, 0, 0);
}
