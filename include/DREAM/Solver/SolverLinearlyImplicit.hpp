...
namespace DREAM
{
    ...
    private:
        ...
        // Vector to store source terms in    ( this petsc_S is a global vector which is split among processor)
        Vec petsc_S;

        Vec global_S; // this is a vector own only by the single processor where we store the GLOBAL solution
